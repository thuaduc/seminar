...
\begin{frame}{Refinement: tests and trained critics}
  ...
  \begin{tradeoffs}
    ...
    \con Generated tests and critics only echo the model's blind spots
  \end{tradeoffs}
\end{frame}
...
\begin{frame}{Five systems compared}
  ...
  \begin{tabular}{@{}L{0.14\textwidth}L{0.13\textwidth}L{0.14\textwidth}L{0.15\textwidth}L{0.16\textwidth}L{0.14\textwidth}@{}}
    \toprule
    \textbf{System}
      & \textbf{Orchestration}
      & \textbf{Interface}
...
\begin{frame}{1. Nothing checks what you meant}
  \label{fr:ambiguity}
  \begin{itemize}
    \item The agent commits to \textbf{one reading} of the issue, and nothing
      ever contradicts it.
      \medskip
    \item A wrong guess about something the tests never check
      \textbf{scores as a success}.
  \end{itemize}
...
\begin{frame}{2. It has to guess what to read}
  \begin{itemize}
    \item The repository \textbf{does not fit in context}. Choosing what to
      read is most of the problem.
      \medskip
    \item Hard tasks are disproportionately \textbf{multi-file}, which is exactly
      where agents stall (frame~\ref{fr:numbers}).
  \end{itemize}
...
\begin{frame}{Takeaways}
  \begin{itemize}
    ...
    \item \textbf{Verification is the bottleneck.} In a randomised trial,
      developers were 19\,\% \emph{slower}, and believed they were 20\,\%
