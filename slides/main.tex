...
% +/- trade-off bullets, used on the generation-strategy slides.
\newcommand{\pro}{\item[\textcolor{TUMGreen}{\textbf{+}}]}
\newcommand{\con}{\item[\textcolor{TUMExtRed}{\textbf{--}}]}
...
\newcommand{\agentloopdiagram}[1][none]{%
  ...
  \begin{tikzpicture}[
    ...
    \node[key] at (0.78,-3.72)
      {\textbf{Interface}: connect LLM and system};
  \end{tikzpicture}%
}
...
\begin{frame}{How we write code has changed}
  ...
  \begin{tabular}{@{}L{0.16\textwidth}L{0.30\textwidth}L{0.44\textwidth}@{}}
    \toprule
      & \textbf{Machine} & \textbf{Human} \\
    \midrule
    Manual
      & Compiles and runs it
      & Writes every line; owns the design, and leans on the test suite to
        catch mistakes \\
    \addlinespace
    Autocomplete \newline {\scriptsize 2021--}
      & Suggests the next lines
      & Accepts or rejects each suggestion, line by line, staying the author \\
    \addlinespace
    One-shot \newline {\scriptsize 2022--}
      & Writes a whole function
      & Specifies the task, then must read and verify all of it by hand \\
    \addlinespace
    \textbf{Agentic} \newline {\scriptsize 2024--}
      & Finds files, edits, runs tests
      & States intent up front, steers midway, and reviews the final outcome
...
%   \medskip
%   \textbf{SWE-bench Verified}: 500 human-screened GitHub issues, graded by the
% project's own tests.
...
\begin{frame}{What this talk shows}
  \begin{enumerate}
    ...
    \item \textbf{How do we measure performance?} \\
      ...
      \medskip
    \item \textbf{What are the current challenges?}
  \end{enumerate}
\end{frame}
...
\begin{frame}{A few terms, up front}
  \begin{tabular}{@{}L{0.20\textwidth}L{0.72\textwidth}@{}}
    \toprule
    \textbf{Scaffold} & Everything around the model that lets it act: tools, prompts, control flow \\
    \addlinespace
    \textbf{Tool call} & A single scaffold action the model invokes: open a file, run a command, apply an edit \\
    \addlinespace
    \textbf{Oracle} & Something outside the model that can verify correctness: a
...
\begin{frame}{What is an agentic coding system?}
  ...
  \medskip
  \begin{itemize}
    \item given a natural-language request, \textbf{autonomously} produces a
    working code change.
      \medskip
    \item navigates files, edits, executes, without step-by-step human guidance.
      \medskip
    \item works across files in a repo.
  \end{itemize}
\end{frame}
...
%   loop runs, and each one acts on a different part of it:
...
%   \medskip
%   Neither is a stage. Both cut \emph{across} all five, and both decide whether
...
%   {\small The \textbf{observe--think--act loop.}}
...
%     ]
%       \node[stage] (obs)   at (0,0)   {Observe\\{\scriptsize environment}};
%       \node[stage] (think) at (3.2,0) {Think\\{\scriptsize reason}};
%       \node[stage] (act)   at (6.4,0) {Act\\{\scriptsize tool call}};
%       \draw[arr] (obs) -- (think);
%       \draw[arr] (think) -- (act);
%       \draw[loop] (act.south) to[out=270, in=270]
%         node[below=1mm, font=\scriptsize, text=TUMOrange]
%         {model loops until done} (obs.south);
%     \end{tikzpicture}
%   \end{center}
...
%   \bigskip
%   {\small On failure it stops or moves on. No arrow goes back.
%   \\ Cheaper, predictable, no recovery.}
% \end{frame}
...
\begin{frame}{Interface}
  ...
  \begin{tabular}{@{}L{0.22\textwidth}L{0.46\textwidth}L{0.22\textwidth}@{}}
    \toprule
    \textbf{Design} & \textbf{How it works} & \textbf{Who controls context} \\
    \midrule
    ACI \newline {\scriptsize purpose-built}~\cite{yang2024sweagent}
      & Scrollable viewer, linter-guarded edits, summarised search
      & The scaffold \\
    \addlinespace
    Code as action~\cite{wang2024codeact}
      & Writes Python or bash in a sandbox; full shell access
      & The model \\
    \bottomrule
  \end{tabular}
\end{frame}
...
\begin{frame}{Interface}
  ...
  \begin{tabular}{@{}L{0.14\textwidth}L{0.40\textwidth}L{0.36\textwidth}@{}}
    \toprule
    & \textbf{Agent-Computer Interface}~\cite{yang2024sweagent}
...
\begin{frame}{Plan: finding the right code}
  ...
  \begin{itemize}
    \item \textbf{Where}: among tens of thousands of functions, which one?
      ...
      \medskip
    \item \textbf{What}: a signature, an interface, an invariant to hold, stated
      concretely enough that the repository can check it.
  \end{itemize}
...
\begin{frame}{Plan: shape}
  ...
  \begin{columns}[T]
    \begin{column}{0.32\textwidth}
      \centering
      {\scriptsize\bfseries Chain of Thought~\cite{wei2022chain}}\\[6pt]
      \begin{tikzpicture}[
        ...
        \node[thought] (c1) {A};
        \node[thought, below=of c1] (c2) {B};
        \node[thought, below=of c2] (c3) {C};
        \node[thought, below=of c3] (c4) {D};
        \draw[arr] (c1) -- (c2);
        \draw[arr] (c2) -- (c3);
        \draw[arr] (c3) -- (c4);
      \end{tikzpicture}
    ...
    \begin{column}{0.34\textwidth}
      \centering
      {\scriptsize\bfseries Tree of Thoughts~\cite{yao2023tree}}\\[6pt]
      \begin{tikzpicture}[
        thought/.style={draw, circle, fill=TUMBlue!10, minimum size=0.6cm,
                        font=\tiny},
        arr/.style={-{Stealth[length=1.2mm]}, semithick},
        ...
        \node[thought] (t1) {A};
        \node[thought, below right=0.5cm and 0.35cm of t1] (t2a) {B$_2$};
        \node[thought, below left=0.5cm and 0.35cm of t1] (t2b) {B$_1$};
        \node[thought, below right=0.5cm and 0.15cm of t2a] (t3a) {C$_3$};
        \node[thought, below left=0.5cm and 0.15cm of t2a] (t3b) {C$_2$};
    ...
    \begin{column}{0.32\textwidth}
      \centering
      {\scriptsize\bfseries Graph of Thoughts~\cite{besta2024graph}}\\[6pt]
      \begin{tikzpicture}[
        thought/.style={draw, circle, fill=TUMBlue!10, minimum size=0.6cm,
                        font=\tiny},
        arr/.style={-{Stealth[length=1.2mm]}, semithick},
        ...
        \node[thought] (g1) {A};
        \node[thought, below right=0.5cm and 0.35cm of g1] (g2a) {B$_2$};
        \node[thought, below left=0.5cm and 0.35cm of g1] (g2b) {B$_1$};
        \node[thought, below=1.1cm of g1] (g3) {C};
        \node[thought, below=0.7cm of g3] (g4) {D};
        \draw[arr] (g1) -- (g2a);
        \draw[arr] (g1) -- (g2b);
        \draw[arr, TUMBlue] (g2a) -- (g3);
        \draw[arr, TUMBlue] (g2b) -- (g3);
        \draw[arr] (g3) to[bend right=30]
...
\begin{frame}{Plan: Chain of Thought}
  ...
  \begin{center}
    \begin{tikzpicture}
      \node[draw, rounded corners, fill=TUMOrange!8, inner sep=6pt,
        ...
        {\textit{``Intent: The app crashes when you check out with an empty cart.''}};
    \end{tikzpicture}
  \end{center}
  ...
  \begin{center}
  \begin{tikzpicture}[
    cotstep/.style={draw, rounded corners, fill=TUMBlue!5, inner sep=5pt,
                 text width=0.86\textwidth, font=\ttfamily\scriptsize,
                 align=left},
    planbox/.style={draw, rounded corners, fill=TUMBlue!20, inner sep=5pt,
                     text width=0.86\textwidth, font=\small, align=left},
    arr/.style={-{Stealth[length=1.6mm]}, thick, TUMGrayDark},
  ]
    \node[cotstep] (s1)
      {\textcolor{TUMBlue}{Think:} empty cart, maybe a divide-by-zero in the total.\\
       \textcolor{TUMOrange}{Action:} grep 'def total'\\
       \textcolor{black!55}{Observation:} cart.py:12, return sum(prices) / len(items)};
    \node[cotstep, below=13pt of s1] (s2)
      {\textcolor{TUMBlue}{Think:} len(items) is 0 when the cart is empty.\\
       \textcolor{TUMOrange}{Action:} open cart.py:12 to confirm the division\\
       \textcolor{black!55}{Observation:} yes, no empty-case guard};
    \node[planbox, below=13pt of s2] (s3)
      {\textbf{Plan.}\quad \emph{where:} cart.py:12 \texttt{total()}\quad
...
\begin{frame}{Plan: Tree of Thoughts}
  ...
  learned value for \textbf{+23\%}, at the price of a model call per
...
\begin{frame}{Generate: infilling}
  \label{fr:gen-infill}
  {\small The scaffold opens a \textbf{gap}; the model fills it.}

  \medskip
  \begin{center}
  \begin{tikzpicture}
    \node[draw, rounded corners, fill=TUMBlue!5, inner sep=8pt,
          text width=0.78\textwidth, font=\ttfamily\small, align=left]
      {\textcolor{black!55}{def total(items):}\\
       \textcolor{black!55}{~~~~prices = [i.price for i in items]}\\
       \textcolor{TUMOrange}{~~~~if not items:}\\
       \textcolor{TUMOrange}{~~~~~~~~return 0}\\
       \textcolor{black!55}{~~~~return sum(prices) / len(items)}};
  \end{tikzpicture}
  \end{center}

  {\footnotesize Reads the \textcolor{black!55}{grey}, writes only the
  \textcolor{TUMOrange}{orange}.}

  \begin{tradeoffs}
    \pro Filling a gap is what models are trained on most
    \pro Changes land in the right place, since the scaffold picks the spot
    \con One span at a time, in practice one function
  \end{tradeoffs}
\end{frame}

\begin{frame}{Generate: diff generation}
  \label{fr:gen-diff}
  {\small Send \textbf{only the change}, not the whole file.}
  ...
  \begin{tikzpicture}
    \node[draw, rounded corners, fill=TUMBlue!5, inner sep=8pt,
      ...
      {\textcolor{black!55}{@@ def total(items): @@}\\
       \textcolor{black!55}{~~~~~prices = [i.price for i in items]}\\
       \textcolor{TUMGreen}{+~~~~if not items:}\\
       \textcolor{TUMGreen}{+~~~~~~~~return 0}\\
       \textcolor{black!55}{~~~~~return sum(prices) / len(items)}};
  \end{tikzpicture}
  \end{center}
  ...
  \smallskip
  \begin{tradeoffs}
    \pro Edits a huge file without re-writing it
    \pro One patch, many places, many files
    \con The model must point to \emph{where} the edit goes --- the surrounding
      context lines locate it
    \con Off by one line and the patch will not apply, before any test runs
  \end{tradeoffs}
\end{frame}

\begin{frame}{Generate: sample $N$, pick one}
  \label{fr:gen-sample}
  {\small Write \textbf{$N$ patches}, keep the one that \textbf{passes}.}

  \medskip
  \begin{center}
  \begin{tikzpicture}[
    cand/.style={draw=TUMBlue, fill=TUMBlue!5, rounded corners=3pt,
                 font=\footnotesize, align=center, inner sep=5pt,
                 text width=0.24\textwidth, minimum height=1.0cm},
    pick/.style={draw=TUMOrange, very thick, fill=TUMOrange!12,
                 rounded corners=3pt, font=\footnotesize, align=center,
                 inner sep=5pt, text width=0.24\textwidth, minimum height=1.0cm},
    sc/.style={font=\scriptsize, align=center},
  ]
    \node[cand] (a) at (0,0)               {A: wrap in\\try/except};
    \node[pick] (b) at (0.33\textwidth,0)  {B: if not items:\\return 0};
    \node[cand] (c) at (0.66\textwidth,0)  {C: return 0\\always};
    \node[sc, below=1.5mm of a, text=TUMExtRed]  {tests 2/3};
    \node[sc, below=1.5mm of b, text=TUMGreen]   {tests 3/3, \textbf{pick}};
    \node[sc, below=1.5mm of c, text=TUMExtRed]  {tests 0/3};
  \end{tikzpicture}
  \end{center}

  \smallskip
  \begin{tradeoffs}
    \pro One bad sample no longer sinks the task
    \pro Different candidates try different ideas
    \con Picking needs an \textbf{oracle}; the model's confidence is not one
    \con Cost grows with $N$: calls, dollars, wall-clock
  \end{tradeoffs}
\end{frame}
...
\begin{frame}{Refinement: iterative self-repair}
  ...
  \begin{tabular}{@{}L{0.30\textwidth}L{0.60\textwidth}@{}}
    \toprule
    \textbf{Mechanism} & \textbf{Correction signal} \\
    \midrule
    Self-Refine~\cite{madaan2023selfrefine}
      & The model to critique and rewrite its own output \\
    \addlinespace
    Reflexion~\cite{shinn2023reflexion}
      & Model summarize what went wrong after a failed attempt before trying again. \\
    \addlinespace
    Self-debugging~\cite{chen2024selfdebug}
      & Model explains its own code while also considering the execution results. \\
    \bottomrule
  \end{tabular}

  \smallskip
  \begin{tradeoffs}
    \pro Increase success rate by 2 to 3\%.
    \con Ceiling: if the model could spot its own wrong code just by looking, 
    \\ it would have written it right the first time.
  \end{tradeoffs}
\end{frame}

\begin{frame}{Refinement: tests}
  \label{fr:refine2}
  \small
  \begin{tabular}{@{}L{0.30\textwidth}L{0.60\textwidth}@{}}
    \toprule
    ...
    \midrule
    D4C~\cite{xu2024d4c}
      & Hands the failing tests and the whole function back to the model and asks for a fresh fix. \\
    \addlinespace
    Agentless validate~\cite{xia2024agentless}
      & Uses project's own regression tests and adds generated ones to screen out bad patches. \\
    \addlinespace
  ...
  \smallskip
  \begin{tradeoffs}
    \pro Real tests can surprise the model, catching wrong patches
    \con Tests catch only the cases they run, not a guarantee of correctness
  \end{tradeoffs}
\end{frame}
...
%       \begin{itemize}
%         \item this trusts the test suite as ground truth
%       \end{itemize}
%       \medskip
%     \item \textbf{Diminishing returns}
%       \begin{itemize}
%         \item beyond a few rounds, a fresh sample beats repairing
%           again~\cite{olausson2024selfrepair}
%       \end{itemize}
%       \medskip
%     \item \textbf{Repair vs.\ best-of-$N$}
%       \begin{itemize}
%         \item both just buy more compute to reduce errors; only comparable at
%           an equal budget
%       \end{itemize}
%   \end{itemize}
% \end{frame}
...
\begin{frame}{Five systems compared}
  ...
  \begin{tabular}{@{}L{0.18\textwidth}L{0.16\textwidth}L{0.18\textwidth}L{0.20\textwidth}L{0.18\textwidth}@{}}
    \toprule
    \textbf{System}
      & \textbf{Orchestration}
      & \textbf{Interface}
...
% \begin{frame}{Where are these systems used?}
%   \label{fr:used}
%   {\small The surveyed five are mostly \textbf{research artefacts}; the tools
%   developers use daily are their \textbf{productised descendants}.}

%   \medskip
%   \begin{itemize}
%     \item \textbf{Research \& benchmarks}: SWE-agent, AutoCodeRover and
%       Agentless. Open-source, run mostly on SWE-bench, blueprints for the
%       patterns.
%       \smallskip
%     \item \textbf{Shipped as products}: OpenHands (All Hands AI, open
%       platform), Devin (Cognition, commercial cloud agent).
%       \smallskip
%     \item \textbf{What developers actually use}: Claude Code, Cursor, Copilot,
%       Codex. The \emph{same} observe, think, act loop, the same purpose-built
%       interface, the same execution-grounded refinement.
%   \end{itemize}

%   \medskip
%   {\small Same ingredients, deployed as IDE assistants, autonomous PR and issue
%   bots, and CI jobs.}
% \end{frame}

\section{How do we measure performance?}

\begin{frame}{Which benchmarks do they use?}
  \label{fr:benchmarks}
  \footnotesize
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
      \textbf{Sonnet 5}~\cite{anthropic2026sonnet5}
      \begin{itemize}\itemsep0pt
        \item SWE-bench~\cite{openai2024verified,deng2025swebenchpro,swebench2025multilingual,yang2024swebenchmm}
        \item Terminal-Bench 2.1~\cite{merrill2026terminalbench}
        \item OSWorld-Verified~\cite{xie2024osworld,xlang2025osworldverified}
        \item Humanity's Last Exam~\cite{phan2025hle}
        \item GDPval-AA v2~\cite{aa2026gdpvalaa,patwardhan2025gdpval}
        \item BrowseComp~\cite{wei2025browsecomp}
        \item USAMO 2026~\cite{petrov2025usamo,balunovic2025matharena}
      \end{itemize}
    \end{column}
    \begin{column}{0.5\textwidth}
      \textbf{GPT-5.6}~\cite{openai2026gpt56}
      \begin{itemize}\itemsep0pt
        \item SWE-bench Pro~\cite{deng2025swebenchpro}
        \item Terminal-Bench 2.1~\cite{merrill2026terminalbench}
        \item DeepSWE v1.1~\cite{datacurve2026deepswe}
        \item Agents' Last Exam~\cite{sun2026agentslastexam}
        \item GDPval-AA v2~\cite{aa2026gdpvalaa,patwardhan2025gdpval}
        \item BrowseComp~\cite{wei2025browsecomp}
        \item OSWorld 2.0~\cite{yuan2026osworld2}
        \item Toolathlon~\cite{li2025toolathlon}
        \item MRCR v2 (long context)~\cite{gemini2024mrcr,deepmind2026mrcrv2}
        \item HealthBench Professional~\cite{hicks2026healthbenchpro,arora2025healthbench}
        \item AA~\cite{aa2026intelligenceindex,aa2026codingindex}
      \end{itemize}
    \end{column}
  \end{columns}

  \smallskip
  {\tiny Sources: Sonnet 5 System Card,
  2026-06-30~\cite{anthropic2026sonnet5,vellum2026sonnet5}; GPT-5.6 launch
  eval table~\cite{openai2026gpt56,aa2026codingindex}.}
\end{frame}
...
\begin{frame}{SWE-bench}
  ...
  \smallskip
  {\small \textbf{2{,}294} tasks from \textbf{12} mature Python projects
  (Django, scikit-learn, matplotlib, sympy, \ldots). Real, merged fixes,
  nothing synthetic.}
  ...
  \begin{enumerate}
    \item \textbf{Given}
      \begin{itemize}
        \item the issue text
        \item the repository \emph{before} the merge
      \end{itemize}
      \smallskip
    \item \textbf{Produce}
      \begin{itemize}
        \item a patch: the source fix. The grading tests stay hidden
      \end{itemize}
      \smallskip
    \item \textbf{Graded}
      \begin{itemize}
        \item \emph{fail-to-pass} tests must flip
        \item \emph{pass-to-pass} must not regress
      \end{itemize}
  \end{enumerate}
\end{frame}

% \begin{frame}{SWE-bench: metrics}
%   \medskip
%   \begin{itemize}
%     \item \textbf{Functional correctness}: does it run and pass? The only thing
%       that counts.
%       \smallskip
%     \item \textbf{pass@$k$}: give it $k$ tries, solved if any one
%       passes~\cite{chen2021humaneval}.
%       \smallskip
%     \item \textbf{Resolve rate}: pass@1 over a whole split, solved on the first
%       try.
%       \smallskip
...
\begin{frame}{SWE-bench: the five splits}
  ...
  \medskip
  {\small \textbf{Pro} is the contamination-resistant successor to
  \textbf{Verified}: enterprise and
  \textbf{held-out} repositories (kept off public GitHub), several languages, and
  tasks needing \textbf{large multi-file} changes. The frontier drops from
  $\sim$80\,\% on Verified to $\sim$46\,\%~\cite{deng2025swebenchpro}.}
\end{frame}

\begin{frame}{SWE-bench: how hard is each task?}
  \label{fr:difficulty}
  \textbf{Verified tasks come in three difficulty tiers}
  \medskip
  \begin{center}
    \small
    \begin{tabular}{@{}ll@{}}
  ...
  \begin{center}
    \begin{minipage}[c]{0.56\textwidth}
      ...
      \begin{tabular}{@{}lcc@{}}
        ...
        \midrule
        \textcolor{TUMOrange}{$\blacksquare$}~Easy \ {\small$<$15\,min} & 27.3\,\% & 80.4\,\% \\
        \textcolor{TUMGreen}{$\blacksquare$}~Medium \ {\small$\leq$1\,h} & 10.0\,\% & 62.1\,\% \\
        \textcolor{TUMExtRed}{$\blacksquare$}~Hard \ {\small$>$1\,h} & \phantom{0}0.0\,\% & 20.0\,\% \\
        \bottomrule
      \end{tabular}
    \end{minipage}%
    \hspace{0.5cm}%
...
\section{What are the current challenges?}

\begin{frame}{1. The agent never asks}
  \label{fr:ambiguity}
  \begin{itemize}
    \item It reads the issue \textbf{once}, decides what it means, and
      \textbf{never revisits} that.
      \medskip
    \item If the tests don't cover the misunderstanding, the wrong fix
      \textbf{still passed}.
  \end{itemize}

  \vfill
  {\footnotesize
   No surveyed system asks. Clarification mechanisms exist and
   work~\cite{lahiri2022ticoder,mu2024clarifygpt,vijayvargiya2026ambiguity}.}
\end{frame}

\begin{frame}{2. The repo doesn't fit}
  \begin{itemize}
    \item Before it can be right, it has to \textbf{pick what to look at}.
      That choice is most of the problem.
  \end{itemize}
...
\begin{frame}{3. Passing the tests is not being right}
  \begin{itemize}
    \item The suite is a \textbf{sample} of intended behaviour, not the
      behaviour itself.
      \medskip
    \item ``All tests pass'' is also the \textbf{stopping rule}.
  \end{itemize}

  \vfill
  {\footnotesize
   ``Reward hacking is swamping model intelligence
...
\begin{frame}[noframenumbering]{Backup: can we trust the SWE-bench numbers?}
  \label{fr:trust}
...
\begin{frame}[noframenumbering]{Backup: surveyed systems on SWE-bench Verified}
  ...
  than the Verified numbers above, so not directly comparable. Self-reported,
  and Cognition stopped publishing benchmark numbers in
...
\begin{frame}[noframenumbering]{Backup: how planning points at real code}
...
\begin{frame}[noframenumbering]{Backup: localisation in Agentless}
  {\small Narrowing without letting the model wander: three fixed steps, coarse
  to fine~\cite{xia2024agentless}.}

  \bigskip
  \begin{enumerate}
    \item \textbf{Rank files}: from the repo layout and the issue, shortlist the
      files most likely involved.
      \medskip
    \item \textbf{Rank elements}: inside those files, narrow to specific classes
      and functions.
      \medskip
...
\begin{frame}[noframenumbering]{Backup: localisation in AutoCodeRover}
  {\small Let the model navigate, but through the syntax tree rather than whole
  files~\cite{zhang2024autocoderover}.}
  ...
  \begin{block}{Discovered, not fixed}
    The model chooses what to search. Agentless, by contrast, is a fixed
    funnel.
  \end{block}
\end{frame}
