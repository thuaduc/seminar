...
\begin{frame}{A few terms, up front}
  \begin{tabular}{@{}L{0.20\textwidth}L{0.72\textwidth}@{}}
    ...
    \textbf{Scaffold} & The non-model code around it --- tools, prompts,
      control flow \\
    \addlinespace
    \textbf{Tool call} & The model asking the scaffold to act --- read a file,
      run the tests --- and getting the output back \\
    \addlinespace
...
\begin{frame}{Generate: infilling}
  \label{fr:gen-infill}
  {\small Complete a \textbf{span} from the surrounding context --- works well
  \textbf{within a single function}.}
  ...
  \smallskip
  {\small Conditions on the \textcolor{black!55}{grey context}, emits only the
  \textcolor{TUMOrange}{orange span}.}
\end{frame}

\begin{frame}{Generate: diff generation}
  \label{fr:gen-diff}
  {\small Emit a \textbf{unified diff}, not the whole file --- smaller output,
  but \textbf{fragile if the context is stale}.}
  ...
  \medskip
  {\small Only the change travels --- but the \textcolor{black!55}{context
  lines} must still \textbf{match}, or the patch will not apply.}
\end{frame}
...
\begin{frame}{Generate: sample $N$, pick one}
  ...
  \medskip
  {\small Same plan, $N$ attempts. Ranking needs an \textbf{oracle} --- the
  model's own confidence is not enough.}
\end{frame}
...
\begin{frame}[noframenumbering]{Backup: Sonnet 5 --- what did Anthropic report?}
  \label{fr:sonnet5}
  Released 2026-06-30, ``the most agentic Sonnet yet,'' priced below
  Opus~\cite{anthropic2026sonnet5}.
  ...
  \begin{center}
    ...
    \begin{tabular}{@{}lcl@{}}
      \toprule
      \textbf{Model} & \textbf{SWE-bench Pro} & \textbf{Scored by} \\
      \midrule
      Sonnet 4.6 & 58.1\,\% & {\color{TUMGrayDark}\emph{self-reported}} \\
      Sonnet 5 & 63.2\,\% & {\color{TUMGrayDark}\emph{self-reported}} \\
      Opus 4.8 & 69.2\,\% & {\color{TUMGrayDark}\emph{self-reported}} \\
      \bottomrule
    \end{tabular}
  \end{center}

  \medskip
  {\footnotesize Anthropic's own system card, same simple bash-plus-file-edit
  scaffold used since Claude 3.5~\cite{vellum2026sonnet5}. No independent Pro
  leaderboard has scored Sonnet 5 yet --- same gap as the April
  table~\cite{deng2025swebenchpro}.}

  \medskip
  {\small A Verified score above 80\,\% circulated at launch, but figures
  disagree (82--85\,\%) and none trace to the system card directly ---
  omitted here on purpose.}
\end{frame}

\begin{frame}[noframenumbering]{Backup: GPT-5.6 --- what did OpenAI report?}
  \label{fr:gpt56}
  Preview announced 2026-06-26 --- the \textbf{U.S. government} asked OpenAI
  to cap access to $\sim$20 approved partners first; public release followed
  13 days later, 2026-07-09~\cite{techcrunch2026gpt56gate}.

  \medskip
  Three tiers --- \textbf{Sol} (flagship), \textbf{Terra}, \textbf{Luna}:
  ...
  \begin{center}
    ...
    \begin{tabular}{@{}lccl@{}}
      \toprule
      & \textbf{SWE-bench Pro} & \textbf{Terminal-Bench} & \textbf{Scored by} \\
      \midrule
      Sol & 64.6\,\% & 88.8\,\% & {\color{TUMGrayDark}\emph{self-reported}} \\
      Terra & 63.4\,\% & 87.4\,\% & {\color{TUMGrayDark}\emph{self-reported}} \\
      Luna & 62.7\,\% & 84.7\,\% & {\color{TUMGrayDark}\emph{self-reported}} \\
      \bottomrule
    \end{tabular}
  \end{center}

  \medskip
  {\footnotesize No Verified score published at all --- the eval table leads
  with Pro, the harder split~\cite{openai2026gpt56}.}

  \medskip
  {\small For 13 days, the most capable model on the market had \textbf{no
  public benchmark of any kind} --- access, not just scoring, gates what can
  be verified~\cite{techcrunch2026gpt56gate}.}
\end{frame}
