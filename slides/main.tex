...
\begin{frame}{What this talk shows}
  \begin{enumerate}
    ...
    \item \textbf{How do we measure performance?} \\
      ...
      \medskip
    \item \textbf{Why are we stuck on the hard tasks?} \\
      {\small Four reasons --- and the pattern behind them.}
  \end{enumerate}
\end{frame}
...
\begin{frame}{A few terms, up front}
  \begin{tabular}{@{}L{0.20\textwidth}L{0.72\textwidth}@{}}
    ...
    \textbf{Scaffold} & The non-model code around it --- tools, prompts,
      control flow \\
    \addlinespace
    \textbf{Tool call} & A single scaffold action the model invokes and waits
      on --- open a file, run a command, apply an edit \\
    \addlinespace
    \textbf{Patch} & A description of a change --- which lines, in which
      files --- not the changed code \\
    \addlinespace
    \textbf{Harness} & The fixed environment a submission is graded in ---
      not the one it was made in \\
    \addlinespace
...
\begin{frame}{Generate: infilling}
  \label{fr:gen-infill}
  {\small Complete a \textbf{span} from the surrounding context.}

  \smallskip
  {\small Plan from earlier: guard \texttt{total()} so an empty cart returns 0.}
  ...
  {\footnotesize Conditions on the \textcolor{black!55}{grey context}, emits
  only the \textcolor{TUMOrange}{orange span}.}

  \begin{tradeoffs}
    \pro Matches the pre-training objective --- fill a gap from both sides
    \pro Cannot be mis-placed: the scaffold decides where the gap is
    \con One contiguous span --- in practice, one function
    \con The surrounding code has to fit in the prompt
  \end{tradeoffs}
\end{frame}

\begin{frame}{Generate: diff generation}
  \label{fr:gen-diff}
  {\small Emit a \textbf{unified diff}, not the whole file.}
  ...
  \smallskip
  \begin{tradeoffs}
    \pro Only the change travels --- edits a large file without re-emitting it
    \pro One patch spans many hunks and many files
    \con The model must state \emph{where}: the
      \textcolor{black!55}{context lines} have to match
    \con A stale anchor is rejected before a single test runs
  \end{tradeoffs}
\end{frame}

\begin{frame}{Generate: sample $N$, pick one}
  \label{fr:gen-sample}
  {\small Generate \textbf{many candidates}; rank by \textbf{tests or a critic}.
  AlphaCode samples up to \emph{millions}, clusters by
  ...
  \begin{tradeoffs}
    ...
    \pro Candidates differ --- covers approaches a single pass would miss
    \con Ranking needs an \textbf{oracle}; the model's own confidence is not
      enough
    \con Cost scales with $N$ --- calls, dollars, wall-clock
  \end{tradeoffs}
\end{frame}
...
\begin{frame}{Where are these systems used?}
  ...
  \medskip
  \begin{itemize}
    \item \textbf{Research \& benchmarks} --- SWE-agent, AutoCodeRover,
    ...
    \item \textbf{Shipped as products} --- OpenHands (All Hands AI, open
      ...
      \smallskip
    \item \textbf{What developers actually use} --- Claude Code, Cursor,
      Copilot, Codex: the \emph{same} observe--think--act loop, purpose-built
      interface, and execution-grounded refinement.
  \end{itemize}

  \medskip
  {\small Same ingredients --- deployed as IDE assistants, autonomous PR/issue
  bots, and CI jobs.}
\end{frame}

\section{How do we measure performance?}

\begin{frame}{Sonnet 5 --- which benchmarks?}
  \label{fr:sonnet5}
  \begin{columns}[t]
    \begin{column}{0.5\textwidth}
      \begin{itemize}
        \item SWE-bench Verified~\cite{openai2024verified}
        \item SWE-bench Pro~\cite{deng2025swebenchpro}
        \item SWE-bench Multilingual~\cite{swebench2025multilingual}
        \item SWE-bench Multimodal~\cite{yang2024swebenchmm}
  ...
  \bigskip
  {\scriptsize Source: Sonnet 5 System Card,
  2026-06-30~\cite{anthropic2026sonnet5,vellum2026sonnet5}.}
\end{frame}

\begin{frame}{GPT-5.6 --- which benchmarks?}
  \label{fr:gpt56}
  \begin{columns}[t]
    \begin{column}{0.5\textwidth}
      \begin{itemize}
    ...
    \begin{column}{0.5\textwidth}
      \begin{itemize}
        ...
        \item AA Intelligence Index~\cite{aa2026intelligenceindex}
        \item AA Coding Agent Index~\cite{aa2026codingindex}
      \end{itemize}
    \end{column}
  \end{columns}

  \bigskip
  {\scriptsize Source: GPT-5.6 launch eval
  table~\cite{openai2026gpt56,aa2026codingindex}.}
\end{frame}
...
\begin{frame}{SWE-bench: the five splits}
  ...
  \medskip
  {\small \textbf{Pro} is the contamination-resistant successor: enterprise and
...
\section{Why are we stuck on the hard tasks?}

\begin{frame}{1. Nothing checks what you meant}
  \label{fr:ambiguity}
  \begin{itemize}
    \item The agent commits to \textbf{one reading} of the issue --- and
...
\begin{frame}{2. It has to guess what to read}
  \begin{itemize}
    \item The repository \textbf{does not fit in context}. Choosing what to
      ...
      \medskip
    \item Hard tasks are disproportionately \textbf{multi-file} --- exactly
