...
\begin{frame}{Orchestration}
  ...
  \bigskip
  Two commons ways to run the same five stages.
\end{frame}
...
\begin{frame}{Orchestration: fixed pipeline}
  \label{fr:orch-fixed}
  {\small The \textbf{scaffold} is in control: the developer fixes the steps in
...
\begin{frame}{Interface: side by side}
  \label{fr:iface}
  \small
...
\begin{frame}{Back to the five stages}
  ...
  \scalebox{0.9}{%
    \begin{tikzpicture}[
      ...
      \node[stage] (intent) at (0,0)    {Intent\\capture};
      \node[here]  (plan)   at (2.6,0)  {Planning};
      \node[stage] (gen)    at (5.2,0)  {Code\\generation};
      \node[stage] (exec)   at (7.8,0)  {Execution};
      \node[stage] (refine) at (10.4,0) {Refinement};
      \draw[arr] (intent) -- (plan);
      \draw[arr] (plan) -- (gen);
      \draw[arr] (gen) -- (exec);
      \draw[arr] (exec) -- (refine);
  ...
  \bigskip
  Orchestration and interface cut \emph{across} all five. Now, stage by
  stage --- starting where the model has the \textbf{least to lean on}.
\end{frame}
...
\begin{frame}{Plan: Chain of Thought}
  \label{fr:bu-cot}
  {\small The \textbf{agentic loop} from orchestration (Slide~\ref{fr:orch}) ---
  \emph{observe, think, act} --- in action. What every production agent runs
  (SWE-agent, OpenHands): \textbf{Thought} $\rightarrow$ \textbf{Action}
  $\rightarrow$ \textbf{Observation}, repeated~\cite{yao2023react}.}

  \medskip
  \begin{center}
  \begin{tikzpicture}
    \node[draw, rounded corners, fill=TUMBlue!5, inner sep=8pt,
          text width=0.9\textwidth, font=\ttfamily\scriptsize, align=left]
      {\textcolor{TUMBlue}{Thought:} CSV breaks on a comma --- the writer isn't quoting.\\
       \textcolor{TUMOrange}{Action:} search\_dir 'def write\_row'\\
       \textcolor{black!55}{Obs:} export/csv.py:41\\[2pt]
       \textcolor{TUMBlue}{Thought:} let me read it.\\
       \textcolor{TUMOrange}{Action:} open export/csv.py:41\\
       \textcolor{black!55}{Obs:} def write\_row(cols): return ','.join(cols)\\[2pt]
       \textcolor{TUMBlue}{Thought:} manual join, no quoting --- use csv.writer.\\
       \textcolor{TUMOrange}{Action:} edit export/csv.py:41 \ldots};
  \end{tikzpicture}
  \end{center}

  \medskip
  {\small One straight line. If the first \textbf{Thought} is wrong --- the bug's
  really in the \emph{reader} --- it just keeps going. Only \textbf{execution
  feedback later} catches it, not the shape.}
\end{frame}
...
\begin{frame}{Localisation: Agentless}
...
\begin{frame}{Localisation: AutoCodeRover}
